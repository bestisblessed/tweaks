...
function installStyle() {
  ...
  style.textContent = `
    ...
    [${VIEW_ATTR}="root"] {
      --codexpp-project-home-header-offset: 0px;
      display: flex;
      flex-direction: column;
      flex: 1;
      min-height: 0;
      box-sizing: border-box;
      padding-top: var(--codexpp-project-home-header-offset);
      overflow: hidden;
...
function syncHeaderForProjectHome(state) {
  ...
  if (!(header instanceof HTMLElement)) {
    resetProjectHomeContentOffset(state);
    return;
  }
  ...
  syncProjectHomeContentOffset(state, header);
}

function syncProjectHomeContentOffset(state, header) {
  const root = state.view;
  const host = state.host instanceof HTMLElement && state.host.isConnected
    ? state.host
    : routeContentHost();
  if (!(root instanceof HTMLElement) || !(host instanceof HTMLElement) || !(header instanceof HTMLElement)) return;
  const headerRect = header.getBoundingClientRect();
  const hostRect = host.getBoundingClientRect();
  const offset = Math.max(0, Math.ceil(headerRect.bottom - hostRect.top));
  const next = `${offset}px`;
  if (root.style.getPropertyValue("--codexpp-project-home-header-offset") !== next) {
    root.style.setProperty("--codexpp-project-home-header-offset", next);
  }
}

function resetProjectHomeContentOffset(state) {
  if (
    state.view instanceof HTMLElement &&
    state.view.style.getPropertyValue("--codexpp-project-home-header-offset")
  ) {
    state.view.style.removeProperty("--codexpp-project-home-header-offset");
  }
}
...
function renderProjectHomeHeader(state, node) {
  ...
  controls.className = "no-drag flex shrink-0 items-center gap-2";
  markHeaderInteractive(controls);
  controls.append(
...
function headerIconButton(label, icon, onClick) {
  ...
  button.type = "button";
  button.className =
    "no-drag cursor-interaction flex size-8 items-center justify-center rounded-md text-token-description-foreground hover:bg-token-list-hover-background hover:text-token-foreground focus-visible:outline-token-border focus-visible:outline-2";
  button.setAttribute("aria-label", label);
  button.title = label;
  markHeaderInteractive(button);
  button.innerHTML = icon;
...
function markHeaderInteractive(node) {
  if (!(node instanceof HTMLElement)) return;
